...
if n_section == 3:
 iz=int(nz/2+nz/6)
 zlevel=zmin+(iz-1)*dz
 cset = ax.contourf(xx, yy, speed[iz,:,:], levels=value, zdir='z', offset=zlevel, cmap=cm.seismic, vmin=vmin,vmax=vmax, alpha=0.2, extend='neither')
 iz=int(nz/2-nz/6)
 zlevel=zmin+(iz-1)*dz
 cset = ax.contourf(xx, yy, speed[iz,:,:], levels=value, zdir='z', offset=zlevel, cmap=cm.seismic, vmin=vmin,vmax=vmax, alpha=0.2, extend='neither')
 iz=int(nz/2)
...
plt.savefig("tomo_3D.pdf",dpi=1200,format='pdf')
plt.show()
