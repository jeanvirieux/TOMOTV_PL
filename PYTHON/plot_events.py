...
#ax.plot_wireframe(xx, yy, speed[60,:,:], color='b')
plt.colorbar(cset)
...
while True:
 ...
 if ilist % 1 == 0:  # modulo 10
   result=re.findall(r"[-+]?\d*\.\d+|\d+", line)  # extract real numbers from the string
#   print(' coords',float(result[0]),float(result[1]),float(result[2]))
   ax.scatter3D(float(result[0])/scale,float(result[1])/scale,float(result[2])/scale,c='green',marker='x',s=20,edgecolors='none')
...
plt.savefig("tomo_events.pdf",dpi=1200,format='pdf')
plt.show()
