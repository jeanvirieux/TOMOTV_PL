...
open(7,file='modelS',access='direct',recl=4*nx*ny*nz)
...
open(8,file='modelS',access='direct',recl=4*nx*ny*nz)
write(8,rec=1) modv_2_5d
close(8)

!======================== for plotting with ximage
open(9,file='modelS.2d',access='direct',recl=4*nx*nz)
write(9,rec=1) ((modv_2d(ix,iz),iz=1,nz),ix=1,nx)
close(9)

! ##################################### making the 2.5d medium 
! ##################################### ascii format
open(10,file='modelS.asc')
do iz=2,nz-1
do ix=2,nx-1
