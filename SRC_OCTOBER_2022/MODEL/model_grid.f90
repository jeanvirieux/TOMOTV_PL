...
! module : model_grid
! ==============
! purpose : multigrid strategy for inversion
!-----------------------------------------------------------------------
...
program model_grid
  ...
  read(finput,'(a)') carac
  read(finput,*) chx
  write(flog,*) ' 1 pour ondes P, 2 pour ondes P et S',chx
  ! ####################################  input
  read(finput,'(a)') carac
...
!  write(flog,*) ' (x1,x2,x3) origin of the forward grid ',x1_fwd,x2_fwd,x3_fwd
  read(finput,'(a)') carac    ! ONLY DIFFERENCE COMPARED TO THE FORWARD SAMPLING
  read(finput,*) h1,h2,h3
  write(flog,*) ' grid for the grid h1=h2=h3 ',h1,h2,h3
...
!   ixo=1;iyo=1;izo=1;nix=n1inv;niy=n2inv;niz=n3inv
  ...
  open(17,file='average-layer_vp',access='direct',recl=4*n1inv*n2inv*n3inv)       ! JEAN new file
