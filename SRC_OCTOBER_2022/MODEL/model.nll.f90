...
program model
  ...
  integer(kind=4)                    :: ix,iy,iz,itmoy,icount
  ...
  open(8,file='modelP.nll',access='direct',recl=4)
  !------------ if we have selected P & S velocities simultaneously
  if(chx.eq.2) then
     ...
     open(10,file='modelS.nll',access='direct',recl=4)
  endif
  ...
  ! #####################################  writing the forward model for nll reverse order
  icount=0
  do ix=1,n1
     do iy=1,n2
        do iz=1,n3
           icount=icount+1
           write(8,rec=icount) h1/modforwp(ix,iy,iz)  ! slowness * h
        enddo
     enddo
  enddo
  ...
  if(chx == 2) then
     ...
     do ix=1,n1
        do iy=1,n2
           do iz=1,n3
              icount=icount+1
              write(10,rec=icount) h1/modforws(ix,iy,iz)   ! slowness * length
           enddo
        enddo
     enddo
