...
/*                                                                           */
/* CALLING TIME_3D FROM A PROGRAM WRITTEN IN FORTRAN :                       */
/*                                                                           */
...
/* See also: Tryggvason & Bergman, Geophys.J.Int. 165, 432-435, 2006.        */
/*---------------------------------------------------------------------------*/
#include <stdio.h>
#include <stdlib.h>
#include <math.h>
...
#define    INFINITE    0.500e+19   /* WE DO NOT USE INFINITY AS UNIX DID */
#define    FD_HUGE     0.499e+19
#define    ISINF(x)    ((x)>FD_HUGE)
/* Note: INFINITE must be lower than the SQUARE ROOT of the highest */
/* acceptable float value (machine dependent) to prevent overflow.  */
/* FD_HUGE must be chosen such that INFINITE>FD_HUGE is true.       */
...
static char *err_msg[]=
            {
                ...
                "\ntime_3d: [Bug] macros INFINITE, FD_HUGE not properly set.\n",
...

{
    ...
    if(!(ISINF(INFINITE))) return ERR_INFBUG;
/* if you encounter this error, it probably means you played */
/* around with the values of macros FD_HUGE and INFINITE !   */
...
/* initialize all times as INFINITE if licit point source */
    if(fxs>=0.0 && fxs<=nx-1 && fys>=0 && fys<=ny-1 && fzs>=0 && fzs<=nz-1)
        for(x=0,pf=t_buf;x<nt;x++) *pf++=INFINITE;

/* assign INFINITE to hs in dummy meshes (x=nmesh_x|y=nmesh_y|z=nmesh_z) */
/* and keep masked values in hs_keep[] (will be restored in free_ptrs()) */
    ...
    for(x=0;x<nx;x++){
        for(y=0;y<ny;y++) {
            *pf++=hs[x][y][nmesh_z];
            hs[x][y][nmesh_z]=INFINITE;
        }
        for(z=0;z<nmesh_z;z++) {
            *pf++=hs[x][nmesh_y][z];
            hs[x][nmesh_y][z]=INFINITE;
        }
    }
    for(y=0;y<nmesh_y;y++)
        for(z=0;z<nmesh_z;z++) {
            *pf++=hs[nmesh_x][y][z];
            hs[nmesh_x][y][z]=INFINITE;
        }
...
/* build the re-discretized local model and the associated source position */
    for(i=0;i<N_INIT;i++) HS[i]=T[i]=INFINITE;
    nx=ny=nz=N_INIT_X;
...
/* if relevant, retrieve INFINITE-masked hs values at model boundaries */
...
/* We now propagate bulk and head waves into the region of interest. */
    ...
    for(y=y_start;y<y_end;y++){
        for(z=z_start;z<z_end;z++){
            ...
            else hs_ubb=hs_ube=hs_ueb=INFINITE;
    ...
    for(y=y_start;y>y_begin;y--){
        for(z=z_start;z<z_end;z++){
            ...
            if(x_sf>=0 && x_sf<nmesh_x){
                ...
                else hs_ubb=INFINITE;
                hs_ueb=hs[x_sf][y-1][z];
            }
            else hs_ubb=hs_uee=hs_ueb=INFINITE;
    ...
    for(y=y_start;y>y_begin;y--){
        for(z=z_start;z>z_begin;z--){
            ...
            if(x_sf>=0 && x_sf<nmesh_x){
                ...
                else hs_ube=INFINITE;
                if(z>1) hs_ueb=hs[x_sf][y-1][z-2];
                else hs_ueb=INFINITE;
            }
            else hs_uee=hs_ube=hs_ueb=INFINITE;
...
{    int
    ...
    for(y=y_start;y<y_end;y++){
        for(z=z_start;z>z_begin;z--){
            ...
            else hs_ubb=hs_ube=hs_uee=INFINITE;
