...
! ###########################################################################  
  subroutine sub_new_model(v,s,n1,n2,n3,ni1,ni2,ni3,ixo,iyo,izo,d_max,iopt)
    ...
    integer(kind=4) :: nrepeat,ir1,ir2,ir3,ir
    integer(kind=4) :: iopt_smt   ! 0 no, 1 first, 2 second
    logical :: file_exists_smt

    !======================================================= denoising
    real(kind=4), dimension(:,:,:), allocatable :: x_ns,x_dns
    real(kind=4) :: epsilon,delta,epsilon_noise
    integer(kind=4) :: iopt_dns     ! 0 no, 1 first, 2 second
    logical :: file_exists_dns

    logical :: file_exists_reg
    integer(kind=4) :: iter_max
    real(kind=4) :: xwater

    noffset=0
    if(iopt /= 0) then       ! S velocity or V parameter   iopt=uvs is used
       noffset=ni1*ni2*ni3
    endif

    INQUIRE(FILE="regular.head", EXIST=file_exists_reg)
    if(file_exists_reg) then
       open(71,file='regular.head',status='old')
       read(71,*) iopt_smt,iopt_dns
       close(71)
    else
       iopt_smt=1;iopt_dns=1    ! leave the decision to the file existence ...
    endif

    INQUIRE(FILE="smoothing.head", EXIST=file_exists_smt)
    if(iopt_smt == 0) file_exists_smt=.false.
    INQUIRE(FILE="denoising.head", EXIST=file_exists_dns)
    if(iopt_dns == 0) file_exists_dns=.false.

    !===============================================================
    ! perform smoothing to the slowness perturbation
    !===============================================================

    if(iopt_smt == 1) then
       if(file_exists_smt) then
          open(71,file='smoothing.head',status='old')
          read(71,*) nrepeat,ir1,ir2,ir3
          close(71)
          write(*,*) ' application of a tensorial smoothing on slowness increment'
          write(*,*) ' repetition of the total loop ',nrepeat
          write(*,*) ' nearby nodes in three directions ',ir1,ir2,ir3

          do ir=1,nrepeat
             ! smoothing along x
             allocate(input(ni1)); allocate(output(ni1))
             do k=1,ni3               ! loop over invertable nodes
                do j=1,ni2
                   do i=1,ni1
                      ...
                      input(i)=s(itot)
                   enddo
                   call triangle(ir1,ni1,input,output)
                   do i=1,ni1
                      itot=i+ni1*(j-1)+ni1*ni2*(k-1)+noffset
                      s(itot)=output(i)
                   enddo
             ...
             ! smoothing along y
             allocate(input(ni2)); allocate(output(ni2))
             do k=1,ni3               ! loop over invertable nodes
                do i=1,ni1
                   ...
                   do j=1,ni2
                      ...
                      input(j)=s(itot)
                   enddo
                   call triangle(ir2,ni2,input,output)
                   do j=1,ni2
                      itot=i+ni1*(j-1)+ni1*ni2*(k-1)+noffset
                      s(itot)=output(j)
                   enddo
             ...
             deallocate(input); deallocate(output)
             ! smoothing along z
             allocate(input(ni3)); allocate(output(ni3))
             do j=1,ni2               ! loop over invertable nodes
                do i=1,ni1
                   ...
                   do k=1,ni3
                      ...
                      input(k)=s(itot)
                   enddo
                   call triangle(ir3,ni3,input,output)
                   do k=1,ni3
                      itot=i+ni1*(j-1)+ni1*ni2*(k-1)+noffset
                      s(itot)=output(k)
                   enddo
    ...
    !write(*,*) ' running updating ',ixo,iyo,izo,noffset,n1,n2,n3,ni1,ni2,ni3
...
!!!====================================================================
!!! beginning of 3D denoising of the inverted velocity : only one pass
!!!====================================================================
    if(iopt_dns == 1) then
       
       if(file_exists_dns) then
          open(71,file='denoising.head',status='old')
          read(71,*) epsilon,delta,epsilon_noise
          read(71,*) iter_max,xwater
          close(71)
          write(*,*) ' application of a TV denoising'
          write(*,*) ' epsilon,delta,noise ',epsilon,delta,epsilon_noise
          ! 3D denoising 
          allocate(x_ns(ni1,ni2,ni3)); allocate(x_dns(ni1,ni2,ni3))
          do k=1,ni3               ! loop over invertable nodes
             do j=1,ni2
                do i=1,ni1
                   x_ns(i,j,k)=v(ixo-1+i,iyo-1+j,izo-1+k)
                enddo
             enddo
          enddo

          call nesterov(x_ns,x_dns,ni1,ni2,ni3,epsilon,delta,epsilon_noise,iter_max,xwater)

          do k=1,ni3               ! loop over invertable nodes
             do j=1,ni2
                do i=1,ni1
                   v(ixo-1+i,iyo-1+j,izo-1+k)=x_dns(i,j,k)   ! TV output
                enddo
             enddo
          enddo
          deallocate(x_ns);deallocate(x_dns)
       endif

       !=============================================
       ! apply of a specific smoothing after denoising the velocity model
       !=============================================

       nrepeat=3  ! hardcoded for this specific smoothing
       ir1=2
       ir2=2
       ir3=2
       ...
       do ir=1,nrepeat
          ...
          do k=1,ni3               ! loop over invertable nodes
             do j=1,ni2
                ...
                do i=1,ni1
                   input(i)=v(ixo-1+i,iyo-1+j,izo-1+k)
                enddo
                call triangle(ir1,ni1,input,output)
                do i=1,ni1
                   v(ixo-1+i,iyo-1+j,izo-1+k)=output(i)
                enddo
          ...
          do k=1,ni3               ! loop over invertable nodes
             do i=1,ni1
                ...
                do j=1,ni2
                   input(j)=v(ixo-1+i,iyo-1+j,izo-1+k)
                enddo
                call triangle(ir2,ni2,input,output)
                do j=1,ni2
                   v(ixo-1+i,iyo-1+j,izo-1+k)=output(j)
                enddo
          ...
          do j=1,ni2               ! loop over invertable nodes
             do i=1,ni1
                ...
                do k=1,ni3
                   input(k)=v(ixo-1+i,iyo-1+j,izo-1+k)
                enddo
                call triangle(ir3,ni3,input,output)
                do k=1,ni3
                   v(ixo-1+i,iyo-1+j,izo-1+k)=output(k)
                enddo
       ...
       !=============================================
       ! end of hardcoded smoothing when denoising is applied
       !=============================================

    endif

    !=============================================
    ! end of denoising on the velocity model
    !=============================================
...
!     update position 
    ...
    ELSE
      signe=sign(1.,dp_z)
      p_z(isrc)=p_z(isrc) + signe*dvert_max
    ENDIF
  endif
...
!     call in_out
  ...
  yp=p_y(isrc)
  zp=p_z(isrc)
! MODIF STEPHANIE POUR QUE LA POSITION LES SOURCES AU-DESSUS DE LA TOPO NE SOIT PAS MODIFIEE SI LE CHOIX SOURCE OUT A ETE VALIDE DANS MODEL.TOMO.TOPO
  if(ki_m(isrc) /=0) then
    call in_out(xo,yo,zo,h1,h2,h3,n1,n2,n3,xp,yp,zp,iout)
    ki_m(isrc)=iout
    if(ki_m(isrc) == 0) write(*,*) 'WARNING LOST ONE SOURCE',ki_m(isrc),isrc,xp,yp,zp 
!  ELSE
!	write(*,*) 'SOURCE OUTSIDE AT THE PREVIOUS ITERATION',ki_m(isrc),isrc 
  endif
enddo
...
!STEPHANIE  if(ki_pos(isrc) /= 0 .and. ki_to(isrc) /= 0) then
!     earthquakes
    if(ki_m(isrc) == 1) then ! inside
      ...
      kpos=kpos+1   !STEPHANIE   ! increment for further inversion  
      kto=kto+1     !STEPHANIE   ! increment for further inversion   
!      ki_pos(isrc)=0
!      ki_to(isrc)=0  
      irec=irec+1  !CORRECTION STEPHANIE  CORRECT JEAN
      write(usrc,rec=irec+1) p_x(isrc),p_y(isrc),p_z(isrc),p_to(isrc),&
              ki_pos(isrc),ki_to(isrc),ki_m(isrc),id_src(isrc)
    endif
