...
if(nt /= ntp+nts) then
  write(*,*) ' inconsistent error in read_fobs could not survive '
  stop
endif
...
do irec=1,ntp ! lecture des temps P
   ...
   if(temps(irec) < 0.) then
      write(95,*) ' just a warning P negative travel time for initial loc',irec,id_dat(irec),temps(irec)
   endif   
enddo 
...
read(uobs,rec=1+ntp+irec) id_dat(ntp+irec),temps(ntp+irec),dtemps(ntp+irec),lut_src(ntp+irec),lut_sta(ntp+irec),lut_ray(ntp+irec)
  if(temps(irec) < 0.) then
      write(95,*) ' just a warning S negative travel time for initial loc ',irec,id_dat(irec),temps(irec)
   endif
enddo
endif
