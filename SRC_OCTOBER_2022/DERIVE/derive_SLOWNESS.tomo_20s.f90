...
!!!JEAN
                 if(tdat > 20.) then  ! peut-etre une erreur d'arrondi de la minute
                    write(98,*) id_dat(idata),temps(idata),tps_ray,tdat,'sta ID',idsta
                    write(*,*) 'P data id t_obs,t_syn,res > 20 ',id_dat(idata),temps(idata),tps_ray,tdat,'sta ID',idsta
                 endif
                 if(tdat < -20.) then  ! peut-etre une erreur d'arrondi de la minute
                    write(97,*) id_dat(idata),temps(idata),tps_ray,tdat,'sta ID',idsta
                    write(*,*) 'P data id t_obs,t_syn,res < -20 ',id_dat(idata),temps(idata),tps_ray,tdat,'sta ID',idsta
                 endif
...
!!!JEAN
                    if(tdat > 20.) then  ! peut-etre une erreur d'arrondi de la minute
                       write(98,*) id_dat(idata),temps(idata),tps_ray,tdat,'ID sta ',idsta
                       write(*,*) 'S data id t_obs,t_syn,res > 20 ',id_dat(idata),temps(idata),tps_ray,tdat,'ID sta ',idsta
                    endif
                    if(tdat < -20.) then  ! peut-etre une erreur d'arrondi de la minute
                       write(97,*) id_dat(idata),temps(idata),tps_ray,tdat,'ID sta ',idsta
                       write(*,*) 'S data id t_obs,t_syn,res < -20 ',id_dat(idata),temps(idata),tps_ray,tdat,'ID sta ',idsta
                    endif
                    if(temps(idata) < 0.) then
