       ...
       if(file_exists_dns) then
          ...
          do k=1,ni3               ! loop over invertable nodes
             do j=1,ni2
                do i=1,ni1
                   itot=i+ni1*(j-1)+ni1*ni2*(k-1)+noffset
                   x_ns(i,j,k)=s(itot)*0.5
                enddo
             enddo
          enddo

          call nesterov(x_ns,x_dns,ni1,ni2,ni3,epsilon,delta,epsilon_noise)

          do k=1,ni3               ! loop over invertable nodes
             do j=1,ni2
                do i=1,ni1
                   itot=i+ni1*(j-1)+ni1*ni2*(k-1)+noffset
                   s(itot)=2.*x_dns(i,j,k)   ! back the slowness update
                enddo
             enddo
          enddo
