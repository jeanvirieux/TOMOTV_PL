...
! 
  subroutine sub_new_model(v,s,n1,n2,n3,ni1,ni2,ni3,ixo,iyo,izo,d_max,iopt)
    ...
    !===============================================================
    ! check what is the sequence if there is any
    !===============================================================
    ...
    !===============================================================
    ! add smoothing the slowness perturbation
    !===============================================================
    ...
    ! end of smoothing
    !=============================================
    !********* still in the first pass
    !=============================================
    ! beginning of denoising 2D for the moment
    !=============================================
    if(iopt_dns == 1) then
       if(file_exists_dns) then
          ...
          ! 2D denoising over mean of j=2 and j=3 over j=1,4
          allocate(x_ns(ni1,ni3)); allocate(x_dns(ni1,ni3))
          do k=1,ni3               ! loop over invertable nodes
             j=2
             do i=1,ni1
                itot=i+ni1*(j-1)+ni1*ni2*(k-1)+noffset
             ...
             do i=1,ni1
                itot=i+ni1*(j-1)+ni1*ni2*(k-1)+noffset
                x_ns(i,k)=x_ns(i,k)+s(itot)*0.5
             enddo
          enddo

          call nesterov(x_ns,x_dns,ni1,ni3,epsilon,delta,epsilon_noise)

          do k=1,ni3               ! loop over invertable nodes
             do j=1,ni2
                do i=1,ni1
                   itot=i+ni1*(j-1)+ni1*ni2*(k-1)+noffset
                   s(itot)=2.*x_dns(i,k)   ! back the slowness update
                enddo
             enddo
          enddo
