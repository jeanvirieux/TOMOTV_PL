!###############################################################################|
! PROGRAM hypo2fobsfsrc     HYPO71 >>> fobs
!     input  
...
!*********************************************
! read the output file of HYPO71 : ****.prt
!*********************************************
...
call read_fobs_P(string,t_sta,dt,t0_src_d,name,iquality,ierr)
...
! extract date or utime, iquality,
call read_fobs_S(string,t_sta,dt,t0_src_d,name,iquality,ierr)
! check the selection of the data
...
subroutine read_fobs_P(string,t_sta,dt,t0_src_d,name,iquality,ierr)

integer(kind=4) :: iquality,ierr
character*(*) :: string
real(kind=4) :: t_sta
character*(*) :: name
character(len=1) :: pickp,weightp
...
if(iquality == 2 .and. (string(80:80) /= 'A' .or. string(80:80) /= 'B') ) then
  ierr=1
  return
endif

if(iquality == 3 .and. (string(80:80) /= 'A' .or. string(80:80) /= 'B' .or. string(80:80) /= 'C') ) then
  ierr=1
  return
endif
...
read(string,'(23x,a1)') weightp

dt=0.   ! should be computed from weightp
...
subroutine read_fobs_S(string,t_sta,dt,t0_src_d,name,iquality,ierr)

integer(kind=4) :: iquality,ierr
character*(*) :: string
real(kind=4) :: t_sta,dt
character*(*) :: name

character(len=1) :: picks,weights
character(len=23) :: date
...
read(string,'(23x,a1)') weights

! estimation of dt

dt=0.   ! should be computed from weights
